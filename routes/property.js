...
const {uploadPropertyImages,	uploadRiderFiles} = require("../middlewares/file_uploader");
...
router.patch('/:id',
	isLoggedIn,
	isRoleProvider,	uploadRiderFiles.array('property-image', 5),
	async (req, res) => {
	try {
		console.log("hii");
		const {id} = req.params;
		//editedd Boarding bites
		const property = await properties.findById({_id: id});
		console.log(property);
		const imageLink = req.files['property-image'] ? req.files['property-image'][0].path : property.images;
		console.log(imageLink+"byyyyyyyy");

		await properties.findOneAndUpdate({_id: id}, {
			images:imageLink
		});

		if (!property.owner.equals(req.session.userRoleID))
			return res.status(403).send({error: 'Not Authorized!'});
//edit by boarding bites
		let {
		...
		property.images=imageLink;
		//edit by boarding bites
		console.log(property);
		await property.save();
	
		res.send({success: 'property edited successfully',data:property});
	} catch (e) {
		res.render('error', {code: 500, error: 'Internal server error'})
	}
});
